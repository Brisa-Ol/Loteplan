...
const ClientNavbar: React.FC = () => {
  ...
  // ✅ Usamos el AuthContext actualizado
  ...
  // ✅ Validación Segura de Estado
  // Ahora que UserDto tiene 'estado_kyc', TypeScript no se quejará.
  const kycStatus = user?.estado_kyc || 'SIN_INICIAR';
  ...
  // -------------------------------------------------------------
  // HELPER: COLOR MAPPER (Para consistencia visual)
  // -------------------------------------------------------------
                     ...
                     {isAuthenticated ? (
                        <>
                           ...
                           >
                              ...
                              {/* ✅ ITERACIÓN SEGURA DE MENÚ DE USUARIO */}
                              {userNavItems[0]?.submenu?.map((item, idx) => {
                                  ...
                                  const finalColor = getColorValue(item.color || (isLogout ? 'error' : 'inherit'));

                                  return (
                                    <MenuItem 
                                        key={idx} 
                                        onClick={() => { 
                                            // ✅ Seguridad: Si es logout, ejecutar la acción del hook
                                    ...
                                    >
                                        {Icon && (
                                            <ListItemIcon sx={{ color: finalColor, minWidth: 36, mt: item.description ? 0.5 : 0 }}>
                                                <Icon fontSize="small" />
                                            </ListItemIcon>
                                        )}
                                        <Box>
