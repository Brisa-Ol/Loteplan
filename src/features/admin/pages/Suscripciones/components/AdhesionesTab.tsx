...
const ESTADO_CUOTA_CONFIG: Record<
	string,
	{ label: string; color: "default" | "info" | "warning" | "success" | "error" }
> = {
	pendiente: { label: "Pendiente", color: "info" },
	...
	return (
		<Box sx={{ px: 3, pb: 2, bgcolor: alpha(theme.palette.primary.main, 0.03) }}>
			{esPlanEnCuotas && (
				...
				>
					CUOTAS DE ADHESIÓN — Plan: {adhesion.plan_pago.replace("_", " ")} · Total:{" "}
					{formatCurrency(adhesion.monto_total_adhesion)}
				</Typography>
			)}
			<Table size="small" sx={{ mt: esPlanEnCuotas ? 0 : 2 }}>
				...
				<TableBody>
					{pagos.map((pago) => {
						const estadoCfg = ESTADO_CUOTA_CONFIG[pago.estado] ?? {
							label: pago.estado,
							color: "default",
						};
...
const AdhesionRow: React.FC<{
	...
	const estadoCfg = ESTADO_ADHESION_CONFIG[adhesion.estado] ?? {
		label: adhesion.estado,
		color: "default",
	};
	...
	if (adhesion.estado === "completada") {
		...
		const cuotaActual = pagos.find((p) =>
			...
			>
				...
				<TableCell>
					...
					<Typography variant="caption" color="text.secondary">
						{adhesion.usuario?.email}
					</Typography>
				</TableCell>
				<TableCell>
					<Typography variant="body2">
						{adhesion.proyecto?.nombre_proyecto ??
							`Proyecto #${adhesion.id_proyecto}`}
					</Typography>
				</TableCell>
				<TableCell>
				...
				<TableCell>
					<Typography variant="body2">
						{adhesion.cuotas_pagadas} / {adhesion.cuotas_totales}
					</Typography>
				</TableCell>
				<TableCell>
					<Typography
...
const OverduePanel: React.FC<{
	overdue: PagoAdhesionDto[];
	loading: boolean;
}> = ({ overdue, loading }) => {
	const theme = useTheme();
		...
		>
			<Stack direction="row" spacing={1} alignItems="center" mb={1.5}>
				...
				<Typography variant="subtitle2" fontWeight={700} color="error.main">
					{overdue.length} cuota{overdue.length !== 1 ? "s" : ""} de adhesión vencida
					{overdue.length !== 1 ? "s" : ""}
				</Typography>
			</Stack>
			<Stack spacing={1}>
				{overdue.slice(0, 5).map((p: any) => (
					...
					>
						<Typography variant="body2">
							<strong>
								{p.adhesion?.usuario?.nombre} {p.adhesion?.usuario?.apellido}
							</strong>
							{" — "}
		...
		<Box>
			...
			>
				<StatCard
					...
					value={logic.metrics?.total_adhesiones ?? 0}
					icon={<AccountBalance />}
					color="primary"
					loading={logic.loadingMetrics}
				/>
				<StatCard
					title="Completadas"
					value={logic.metrics?.estado_resumen.completadas ?? 0}
					icon={<CheckCircle />}
					color="success"
					loading={logic.loadingMetrics}
				/>
				<StatCard
					title="En Curso / Pendientes"
					value={
						(logic.metrics?.estado_resumen.en_curso ?? 0) +
						(logic.metrics?.estado_resumen.pendientes ?? 0)
					}
					icon={<HourglassEmpty />}
					color="warning"
					loading={logic.loadingMetrics}
				...
				<StatCard
					...
					value={`${logic.metrics?.tasa_cobranza ?? "0"}%`}
					icon={<MoneyOff />}
					color="info"
					loading={logic.loadingMetrics}
				/>
			...
			<QueryHandler
				...
				error={logic.error as Error | null}
			>
				<Paper
					elevation={0}
					sx={{
