...
const DetalleLote: React.FC = () => {
  ...
  const winInfo = useMemo(() => {
    ...
    return {
      ...
      miPujaId,
      montoFinal,
      diferencia: montoFinal - Number(lote.precio_base),
      status,
    };
  }, [lote, user?.id]);
  ...
  const mutationPago = useMutation({
    ...
    onSuccess: (data: any) => {
      if (data.is2FARequired) {
        ...
        showError('No se recibió una URL de pago válida.');
      }
    },
    onError: (err: any) => {
  ...
  const mutationCancelar = useMutation({
    ...
    onError: (err: any) => {
      showError(err.response?.data?.message || err.message || 'Error al retirar la puja');
    },
  });

  const handlePagar = () => {
    if (!winInfo.pujaId) { showError('No se encontró la puja a pagar.'); return; }
    mutationPago.mutate(winInfo.pujaId);
  };

  const handleSolicitarCancelacion = () => {
    confirmDialog.confirm('cancel_puja', {
      monto_puja: winInfo.montoFinal,
      lote: { nombre_lote: lote?.nombre_lote },
    });
  };

  const handleConfirmarCancelacion = () => {
    if (!winInfo.miPujaId) return;
    mutationCancelar.mutate(winInfo.miPujaId);
  };

  if (isLoading) {
    return <Box p={4}><Skeleton variant="rectangular" height={500} sx={{ borderRadius: 4 }} /></Box>;
  }
  if (!lote) {
    return <Alert severity="error">Lote no disponible</Alert>;
  }
  ...
  return (
    <Box sx={{ maxWidth: 1400, mx: 'auto', p: { xs: 2, md: 4 }, pb: 12 }}>
      ...
      <Box sx={{ position: 'relative' }}>
        ...
        {/* Botón de volver en móvil (arriba del header) */}
        ...
        <PageHeader
          ...
          action={
            <Stack direction="row" spacing={2} alignItems="center">
              <Chip
                label={subastaFinalizada ? 'CONCLUIDA' : 'EN VIVO'}
                color={subastaFinalizada ? 'default' : 'error'}
                sx={{ fontWeight: 800, fontSize: '0.9rem', py: 2.5 }}
              />
      ...
      {/* ── Banner ganador (pendiente de pago) ── */}
      {debesPagar && (
        <Fade in timeout={700}>
          <Paper
            elevation={6}
              ...
              <Stack direction="row" spacing={3} alignItems="center">
                ...
                <Box>
                  ...
                  <Typography variant="h6" sx={{ opacity: 0.9 }}>
                    Has asegurado el {lote.nombre_lote}.
                  </Typography>
                  <Typography variant="body2" sx={{ mt: 1, fontWeight: 700 }}>
                    Total a liquidar: {fmt(winInfo.montoFinal)}
                  </Typography>
                </Box>
              </Stack>
              <Button
                ...
                sx={{ color: 'success.main', fontWeight: 900, px: 6, py: 2, fontSize: '1.1rem' }}
              >
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1.8fr 1.2fr' }, gap: 4 }}>
        ...
        <Box>
          <Paper
            elevation={0}
            sx={{
              borderRadius: 4, bgcolor: 'secondary.main',
          ...
          <Card variant="outlined" sx={{ mt: 3, borderRadius: 3, border: 'none', bgcolor: 'background.paper' }}>
            <CardContent>
              <Typography variant="subtitle1" fontWeight={800} display="flex" alignItems="center" gap={1} mb={2}>
                <CalendarMonth color="primary" /> Historial de Subasta
              </Typography>
              <Stack spacing={1}>
          ...
          >
            ...
            <Box
              sx={{
                ...
                bgcolor: alpha(subastaFinalizada ? theme.palette.success.main : theme.palette.error.main, 0.05),
                borderBottom: '1px solid', borderColor: 'divider',
              }}
            >
              {!subastaFinalizada ? (
                <Stack direction="row" justifyContent="center" alignItems="center" spacing={1} color="error.main">
                  <Timer />
                  <CountdownTimer endDate={lote.fecha_fin} />
                </Stack>
              ) : (
                <Typography variant="h6" fontWeight={900} color={winInfo.isWinner ? 'success.main' : 'text.secondary'}>
                  {winInfo.isWinner ? '¡ERES EL GANADOR!' : 'SUBASTA FINALIZADA'}
                </Typography>
              )}
            </Box>
            ...
            <CardContent sx={{ p: 4 }}>
              ...
              <Stack spacing={2} sx={{ bgcolor: 'action.hover', p: 2, borderRadius: 2, mb: 4 }}>
                <Box display="flex" justifyContent="space-between">
                  <Typography variant="body2" color="text.secondary">Precio de Apertura:</Typography>
                  <Typography variant="body2" fontWeight={700}>{fmt(lote.precio_base)}</Typography>
                </Box>
                {winInfo.diferencia > 0 && (
                  <Box display="flex" justifyContent="space-between">
                    <Typography variant="body2" color="primary.main">Oferta Adicional:</Typography>
                    <Typography variant="body2" fontWeight={700} color="primary.main">
                      +{fmt(winInfo.diferencia)}
                    </Typography>
                  </Box>
                )}
                <Divider />
                <Box display="flex" justifyContent="space-between">
                  <Typography variant="subtitle1" fontWeight={900}>TOTAL A PAGAR:</Typography>
              ...
              <Stack spacing={2}>
                {!subastaFinalizada ? (
                  <>
                    ...
                    >
                      {winInfo.isWinner ? 'MEJORAR MI OFERTA' : 'PUJAR AHORA'}
                    </Button>

                    {/* Solo visible si participa y no es el líder actual */}
                    {puedeCancelar && (
                      <Button
                        variant="outlined"
                        color="error"
                        ...
                        startIcon={mutationCancelar.isPending ? <CircularProgress size={18} color="inherit" /> : <Cancel />}
                        sx={{ py: 1.5, fontWeight: 700 }}
                      >
                        {mutationCancelar.isPending ? 'RETIRANDO...' : 'RETIRAR MI PUJA'}
                      </Button>
                    )}
                  </>
                ) : debesPagar ? (
                  <Button
                    ...
                    onClick={handlePagar}
                    disabled={mutationPago.isPending}
                    startIcon={mutationPago.isPending ? <CircularProgress size={20} color="inherit" /> : <ReceiptLong />}
                    sx={{ py: 2.5, fontWeight: 900 }}
                  >
                    {mutationPago.isPending ? 'ABRIENDO PAGO...' : 'PAGAR Y ADJUDICAR'}
                  </Button>
                ) : yaPago ? (
                  <Alert severity="success" variant="filled" sx={{ borderRadius: 2 }}>
                    LOTE ADJUDICADO Y PAGADO
                  </Alert>
                ) : (
              ...
              {Number(lote.excedente_visualizacion) > 0 && (
                ...
                >
                  <Stack direction="row" alignItems="center" spacing={1} mb={0.5}>
                    <AccountBalanceWallet fontSize="small" color="info" />
                    <Typography variant="caption" fontWeight={900} color="info.main">
                      SALDO A FAVOR GENERADO
                    </Typography>
                  </Stack>
                  <Typography variant="h6" fontWeight={800}>
                    {fmt(lote.excedente_visualizacion)}
                  </Typography>
                  <Typography variant="caption" color="text.secondary">
                    El excedente de tu oferta queda acreditado en tu cuenta.
                  </Typography>
                </Box>
              )}
            </CardContent>
      ...
      <PujarModal
        ...
        lote={lote}
        yaParticipa={!!winInfo.miPujaId}
        soyGanador={winInfo.isWinner}
