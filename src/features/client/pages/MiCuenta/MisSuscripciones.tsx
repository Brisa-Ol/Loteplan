...
const MisSuscripciones: React.FC = () => {
	...
	// ── 2. handleConfirmCancel — reemplazar completo:
	// ─────────────────────────────────────────────────
	const handleConfirmCancel = useCallback((inputValue?: string) => {
		if (!confirmDialog.data) return;

		if (confirmDialog.action === "cancel_subscription") {
			const suscripcionId = (confirmDialog.data as SuscripcionDto).id;

			iniciarCancelSuscripcion({ id: suscripcionId, motivo: inputValue ?? '' }, {
				onSuccess: (res) => {
					confirmDialog.close();
					if (res.status === 202 || res.data?.requires2FA || res.data?.requires2FA) {
						setSelectedCancelId(suscripcionId);
						setCancelType("subscription"); // ← guarda el tipo antes de abrir el modal
			...
			iniciarCancelAdhesion(adhesionId, {
				onSuccess: (res) => {
					...
					if (
						...
						setCancelType("adhesion"); // ← guarda el tipo antes de abrir el modal
						setTwoFAError(null);
						twoFaModal.open();
					}
				},
				...
				render: (row) => {
					...
					return (
						<Chip
							...
							icon={
								isComplete ? <CheckCircle /> : isCancelled ? <Cancel /> : <Schedule />
							}
	...
	return (
		<PageContainer maxWidth="lg">
			...
			<ConfirmDialog
				...
				isLoading={
					isCancelling || isInitiatingCancelSuscription || isInitiatingCancel
				}
				title={dialogTitle}
			...
			<TwoFactorAuthModal
				...
				onSubmit={(code) => {
					if (cancelType === "subscription") {
						confirmarCancelSuscripcion(
							{ suscripcionId: selectedCancelId!, codigo_2fa: code },
							{
								onSuccess: () => {
									twoFaModal.close();
									setSelectedCancelId(null);
								...
								onError: (err: any) => {
									setTwoFAError(
										err.response?.data?.message || "Código de seguridad inválido",
									);
								},
							},
						);
						...
						confirmarCancelAdhesion(
							{ adhesionId: selectedCancelId!, codigo_2fa: code },
							{
								onSuccess: () => {
									twoFaModal.close();
									setSelectedCancelId(null);
