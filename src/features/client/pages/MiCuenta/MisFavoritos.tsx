...
const LoteFavoritoCard: React.FC<{
  ...
  // Seleccionar la primera imagen disponible (ya que no hay es_principal en el backend)
  const imagenPrincipal = lote.imagenes?.[0];
  const imagenUrl = ImagenService.resolveImageUrl(imagenPrincipal?.url);

  const formattedPrice = new Intl.NumberFormat(env.defaultLocale, { 
      style: 'currency', 
      currency: env.defaultCurrency, 
      maximumFractionDigits: 0 
  }).format(Number(lote.precio_base));

  const getBadgeConfig = (estado: string) => {
    switch (estado) {
        case 'activa': return { color: 'success' as const, label: 'En Subasta' };
        case 'pendiente': return { color: 'warning' as const, label: 'Próximamente' };
        case 'finalizada': return { color: 'error' as const, label: 'Finalizada' };
        default: return { color: 'default' as const, label: estado };
    }
  };

  const badge = getBadgeConfig(lote.estado_subasta);
    ...
    >
      ...
      <Box sx={{ position: 'relative', height: 220, overflow: 'hidden', bgcolor: 'grey.100' }}>
        ...
        <Chip
          ...
          sx={{ 
              ...
              fontWeight: 700, 
              boxShadow: 2, 
              bgcolor: badge.color === 'default' 
          ? theme.palette.grey[700] // Color para 'default'
          : theme.palette[badge.color as 'success' | 'warning' | 'error'].main,
      color: 'white'
          }}
        />
      ...
      <CardContent sx={{ flexGrow: 1, p: 2.5, pb: 1 }}>
        ...
        <Stack direction="row" spacing={0.5} alignItems="center" mb={2} color="text.secondary">
           ...
           <Typography variant="body2" noWrap sx={{ fontSize: '0.85rem' }}>
               {lote.latitud && lote.longitud 
                 ? `${Number(lote.latitud).toFixed(4)}, ${Number(lote.longitud).toFixed(4)}`
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="flex-end">
            ...
            {/* Si es pendiente mostramos fecha */}
      ...
      {favoritos.length > 0 ? (
        <>
            <Box mb={4} display="flex" alignItems="center" gap={1}>
                <Chip 
                    label={`${favoritos.length} lotes en seguimiento`} 
                    color="primary" 
                    size="small"
