...
import { 
  ...
  Card, CardContent, Divider, Chip, useTheme, IconButton, Avatar, Container, Alert, Fade
} from '@mui/material';
import { 
  AccountBalanceWallet, Warning, TrendingUp, Description, Gavel,
  ChevronRight, CheckCircle, Schedule, Assessment, 
  AccountCircle, Security, MonetizationOn, ArrowForward, ReceiptLong, EmojiEvents
  ...
  return (
    <Box sx={{ bgcolor: 'background.default', minHeight: '100vh', pb: 10 }}>
      ...
      <Box sx={{ 
        ...
        pt: { xs: 4, md: 8 }, pb: { xs: 6, md: 10 }, px: 4,
              ...
              <Box>
                  ...
                  <Fade in={true}>
                    <Alert 
                      ...
                      action={
                        <Button 
                          ...
                          sx={{ fontWeight: 800, color: 'text.primary', bgcolor: 'white' }}
                        >
                          Gestionar Pago
                        </Button>
                      }
                      sx={{ 
                        borderRadius: 2, mb: 4, py: 2, 
                    ...
                    >
                      <Typography variant="subtitle1" fontWeight={800}>
                        {hayPujasUrgentes 
                          ? `¡Acción Requerida! Tienes ${cantidadGanadas} subasta(s) pendiente(s) de pago`
                          : `¡Felicitaciones! Has ganado ${cantidadGanadas} subasta(s)`}
                      </Typography>
                      {pujaMasUrgente && (
                        <Typography variant="body2" sx={{ opacity: 0.9 }}>
                          Lote: {pujaMasUrgente.lote?.nombre_lote} • Quedan <strong>{pujaMasUrgente.diasRestantes} días</strong> para completar el pago.
                        </Typography>
                      )}
                    </Alert>
                  </Fade>
                    ...
                    <CardContent>
                      <Stack spacing={3}>
                        <Stack direction="row" alignItems="center" spacing={1.5}>
                          ...
                          <Typography variant="overline" color={stats.cantidadPagosVencidos > 0 ? 'error.main' : 'text.secondary'} sx={{ fontWeight: 800 }}>Estado de Cuenta</Typography>
                        </Stack>
                        {stats.cantidadPagosVencidos > 0 ? (
                          <Stack spacing={2}>
                            <Box>
                              <Typography variant="h3" fontWeight={800} color="error.main">${totalDeudaVencida.toLocaleString('es-AR')}</Typography>
                              <Chip label={`${stats.cantidadPagosVencidos} CUOTAS VENCIDAS`} size="small" color="error" sx={{ mt: 1, fontWeight: 800 }} />
                            </Box>
                            <Button variant="contained" color="error" fullWidth onClick={() => navigate('/client/finanzas/pagos')} sx={{ fontWeight: 700 }}>Regularizar Deuda</Button>
                          </Stack>
                        ) : (
                          <Typography variant="h5" fontWeight={700} color="success.main">¡Todo al día!</Typography>
                        )}
                      </Stack>
                    </CardContent>
                  </Card>

                  <Card elevation={0} sx={{ borderRadius: 2, border: `2px solid ${theme.palette.divider}` }}>
                    <CardContent>
                      <Stack spacing={3}>
                        <Stack direction="row" alignItems="center" spacing={1.5}>
                        ...
                        {stats.proximoVencimiento ? (
                          <Stack spacing={1.5}>
                            <Typography variant="h3" fontWeight={800}>${Number(stats.proximoVencimiento.monto).toLocaleString('es-AR')}</Typography>
                            <Chip label={`VENCE EL ${new Date(stats.proximoVencimiento.fecha_vencimiento).toLocaleDateString('es-AR')}`} color="primary" variant="outlined" sx={{ fontWeight: 800, width: 'fit-content' }} />
                          </Stack>
                        ) : (
                ...
                <Stack spacing={3}>
                  {resumenes?.map((resumen) => (
                    <Card key={resumen.id} elevation={0} sx={{ borderRadius: 2, border: `2px solid ${theme.palette.divider}`, transition: 'all 0.2s', '&:hover': { transform: 'translateY(-4px)', borderColor: 'primary.main', boxShadow: theme.shadows[4] } }}>
                      <CardContent>
                        <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
                          <Box>
                            <Typography variant="h6" fontWeight={800}>{resumen.nombre_proyecto}</Typography>
                            <Chip label={`${resumen.cuotas_pagadas}/${resumen.meses_proyecto} cuotas`} size="small" variant="outlined" sx={{ mt: 0.5, fontWeight: 700 }} />
                          </Box>
              ...
              <Stack spacing={3}>
                ...
                <Card elevation={0} sx={{ bgcolor: alpha(theme.palette.primary.main, 0.03), border: `2px dashed ${theme.palette.primary.main}`, borderRadius: 2, p: 3, textAlign: 'center' }}>
                  <Stack spacing={1.5} alignItems="center">
                    ...
                    <Button size="small" variant="text" onClick={() => navigate('/client/seguridad')} sx={{ fontWeight: 800 }}>Gestionar 2FA</Button>
                  </Stack>
                </Card>
              </Stack>
