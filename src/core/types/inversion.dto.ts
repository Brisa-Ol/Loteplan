...
/**
 * Dto Respuesta de creacion de Inversion
 */
export interface InversionCreatedResponse {
	message: string;
	inversionId: number;
}

/**
 *DTO para iniciar el pago de una inversion, que requiere el código 2FA para ser validado en el backend. 
 */
export interface InversionStartPaymentDTO {
  inversionId: number;
  codigo_2fa: string;
}

/*
Dto Response de la respuesta del backend al iniciar el proceso de pago de una inversión. 
/*/
export interface InversionStartPaymentResponse {
	message: string
	transaccionId: number;
	redirectUrl: string;
}
