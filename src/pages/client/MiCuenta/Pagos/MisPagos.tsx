...
const getStatusConfig = (status: string) => {
  switch (status) {
    case 'pagado': return { color: 'success', label: 'Pagado', icon: <CheckCircle /> };
    case 'pendiente': return { color: 'info', label: 'Próximo', icon: <Schedule /> };
    case 'vencido': return { color: 'error', label: 'Vencido', icon: <ErrorOutline /> };
    case 'cubierto_por_puja': return { color: 'success', label: 'Cubierto (Puja)', icon: <AccountBalanceWallet /> };
...
const MisPagos: React.FC = () => {
  ...
  const iniciarPagoMutation = useMutation({
    ...
    onError: (err: unknown) => {
      ...
      alert(apiError.message || "Error al iniciar el pago.");
  ...
  const columns: DataTableColumn<PagoDto>[] = useMemo(() => [
    ...
    },
    {
      ...
      // ✅ CORRECCIÓN AQUÍ: 'value as string' soluciona el error de sobrecarga
        ...
        <Card elevation={0} sx={{ p: 3, border: `1px solid ${theme.palette.divider}`, borderRadius: 3, bgcolor: 'background.paper', minWidth: { xs: '100%', md: '80%' } }}>
          <Stack direction={{ xs: 'column', sm: 'row' }} divider={<Divider orientation="vertical" flexItem sx={{ display: { xs: 'none', sm: 'block' }, mx: 2 }} />} spacing={{ xs: 4, sm: 4 }} justifyContent="center" alignItems="center">
            ...
            <Stack direction="row" spacing={2} alignItems="center">
              <Avatar variant="rounded" sx={{ bgcolor: alpha(theme.palette.info.main, 0.1), color: 'info.main', width: 56, height: 56 }}><Schedule fontSize="large" /></Avatar>
              <Box>
                <Typography variant="caption" color="text.secondary" fontWeight={700}>PRÓXIMOS A VENCER</Typography>
                <Typography variant="h5" fontWeight={700} color="text.primary">{formatCurrency(stats.proximosVencimientos)}</Typography>
                <Typography variant="caption" color="text.secondary">{counts.pendientes} cuotas pendientes</Typography>
              </Box>
            </Stack>

            <Stack direction="row" spacing={2} alignItems="center">
              <Avatar variant="rounded" sx={{ bgcolor: alpha(theme.palette.error.main, 0.1), color: 'error.main', width: 56, height: 56 }}><Warning fontSize="large" /></Avatar>
              <Box>
                <Typography variant="caption" color="text.secondary" fontWeight={700}>DEUDA VENCIDA</Typography>
                <Typography variant="h5" fontWeight={700} color="error.main">{formatCurrency(stats.deudaVencida)}</Typography>
                <Typography variant="caption" color="text.secondary">{counts.vencidas} cuotas vencidas</Typography>
              </Box>
            </Stack>
