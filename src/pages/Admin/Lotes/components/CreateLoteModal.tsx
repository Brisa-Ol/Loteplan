...
import type { LoteCreateDTO } from '../../../../types/dto/lote.dto';

interface CreateLoteModalProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: LoteCreateDTO, images: File[]) => Promise<void>;
  isLoading?: boolean;
...
const CreateLoteModal: React.FC<CreateLoteModalProps> = ({
  ...
  const formik = useFormik<LoteCreateDTO>({
    initialValues: {
      nombre_lote: '',
      precio_base: 0,
      fecha_inicio: null,
    ...
    onSubmit: async (values) => {
      try {
        ...
        const dataToSend: LoteCreateDTO = {
          ...values,
