// src/pages/Admin/Proyectos/ProyectoDetailPage.tsx (Corregido)
...
// ❗ CORRECCIÓN 1: Importaciones Faltantes
import { proyectoService } from '../../../Services/proyecto.service';

import { QueryHandler } from '../../../components/common/QueryHandler/QueryHandler';

// (Asumiendo estas rutas para los componentes de las pestañas y modales)
...
import AssignLotesModal from './components/Modals/AssignLotesModal';
import type { AsignarLotesDto, ProyectoDTO, ProyectoUpdateDTO } from '../../../types/dto/proyecto.dto';
...
const AdminProyectoDetailPage: React.FC = () => {
  ...
  // ❗ CORRECCIÓN 2: Convertir el ID a número una sola vez
  ...
  const { data: proyecto, isLoading, error } = useQuery<ProyectoDTO, Error>({
    ...
    // ❗ CORRECCIÓN 2: Usar el ID numérico
    queryFn: () => proyectoService.getProyectoById(proyectoId),
    enabled: !!proyectoId,
  });

  // Mutation: Actualizar proyecto
  const updateMutation = useMutation({
    // ❗ CORRECCIÓN 3: Esperar ID numérico y DTO específico
  ...
  const iniciarProcesoMutation = useMutation({
    // ❗ CORRECCIÓN 3: Esperar ID numérico
    mutationFn: (proyectoId: number) =>
      proyectoService.iniciarProceso(proyectoId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['proyecto', proyectoId] });
      queryClient.invalidateQueries({ queryKey: ['adminAllProjects'] });
    },
  ...
  const assignLotesMutation = useMutation({
    // ❗ CORRECCIÓN 3: Usar tipos correctos y nombre de servicio correcto
    mutationFn: ({ id, data }: { id: number; data: AsignarLotesDto }) =>
      proyectoService.asignarLotes(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['proyecto', proyectoId] });
  ...
  // Handlers
  // ❗ CORRECCIÓN 3: Usar tipos correctos
  const handleEditSubmit = async (proyectoId: number, data: ProyectoUpdateDTO) => {
  ...
  const handleIniciarProceso = () => {
    ...
    if (window.confirm('¿Iniciar el proceso de este proyecto? Esto comenzará el conteo de meses.')) {
      // ❗ CORRECCIÓN 2: Usar el ID numérico
      iniciarProcesoMutation.mutate(proyectoId);
    }
  };
  ...
  const handleFinalizar = () => {
    ...
    if (window.confirm('¿Marcar este proyecto como FINALIZADO? Esta acción es permanente.')) {
      // ❗ CORRECCIÓN 2: Usar el ID numérico
      finalizarMutation.mutate(proyectoId);
    }
  };

  // ❗ CORRECCIÓN 3: Usar tipos correctos
  const handleAssignLotes = async (proyectoId: number, data: AsignarLotesDto) => {
              ...
              <Stack direction="row" spacing={4}>
                <Box>
                  ...
                  <Typography variant="h6" fontWeight="bold">
                    {/* ❗ CORRECCIÓN 4: Convertir a número antes de formatear */}
                    ${Number(proyecto.monto_inversion).toLocaleString() || 0} {proyecto.moneda}
                  </Typography>
                </Box>
