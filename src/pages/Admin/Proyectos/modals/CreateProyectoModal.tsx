...
import SingleImageUpload from '../../../../components/common/singleImageUpload/SingleImageUpload'; // Ajusta ruta si es necesario
...
const CreateProyectoModal: React.FC<CreateProyectoModalProps> = ({ 
  ...
  // ✅ 1. Definimos la función de cierre AQUÍ, al principio del componente
  // para que esté disponible para Formik y para el botón de cerrar.
  const handleClose = () => {
    formik.resetForm();
    setImage(null);
    onClose(); // Llama a la prop del padre
  };
  ...
  const formik = useFormik<CreateProyectoDto>({
    ...
    onSubmit: async (values) => {
      try {
        ...
        // Si todo sale bien, reseteamos y cerramos
        handleClose(); 
      } catch (error) {
  ...
  // Efecto para cambiar moneda automáticamente según el tipo
  useEffect(() => {
