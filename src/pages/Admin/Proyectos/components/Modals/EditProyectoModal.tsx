// src/pages/Admin/Proyectos/components/Modals/EditProyectoModal.tsx
...
import type { ProyectoDTO, ProyectoUpdateDTO } from '../../../../../types/dto/proyecto.dto';

interface EditProyectoModalProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (id: number, data: ProyectoUpdateDTO) => Promise<void>;
...
const validationSchema = Yup.object({
  ...
  monto_inversion: Yup.string().nullable(), // String porque es DECIMAL en backend
  moneda: Yup.string(),
  ...
  latitud: Yup.string().nullable(), // String porque es DECIMAL en backend
  longitud: Yup.string().nullable(), // String porque es DECIMAL en backend
});
...
const EditProyectoModal: React.FC<EditProyectoModalProps> = ({ 
  ...
  const formik = useFormik<ProyectoUpdateDTO>({
    initialValues: {
      nombre_proyecto: '',
      descripcion: '',
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        ) as ProyectoUpdateDTO;
        
        await onSubmit(proyecto.id, cleanedValues);
        onClose();
      } catch (error) {
