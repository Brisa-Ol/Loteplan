// src/components/Admin/Proyectos/CreateProyectoModal.tsx (Corregido)
...
import type { ProyectoCreateDTO } from '../../../../../types/dto/proyecto.dto';
...
interface CreateProyectoModalProps {
  open: boolean;
  onClose: () => void;
  // El tipo 'CreateProyectoDTO' ahora está importado
  onSubmit: (data: ProyectoCreateDTO, image: File | null) => Promise<void>;
  isLoading?: boolean;
}

// ❗ CORRECCIÓN 3: Ajustar validación de 'monto_inversion'
const validationSchema = Yup.object({
  ...
  // Validamos como string, pero chequeamos que sea un número positivo
  monto_inversion: Yup.string()
    .required('Requerido')
    .test(
      'is-positive',
...
const CreateProyectoModal: React.FC<CreateProyectoModalProps> = ({ 
  ...
  const formik = useFormik<ProyectoCreateDTO>({
    // ❗ CORRECCIÓN 4: Ajustar initialValues para que coincidan con el DTO
    initialValues: {
      nombre_proyecto: '',
      descripcion: '',
      ...
      monto_inversion: '0', // DTO espera string
      moneda: 'ARS',
      suscripciones_minimas: 1,
      obj_suscripciones: 1,
      fecha_inicio: '',
      fecha_cierre: '',
      latitud: null, // Usar null en lugar de undefined
      longitud: null, // Usar null en lugar de undefined
    },
    validationSchema: validationSchema,
    onSubmit: async (values) => {
      try {
        // 'values' ya tiene la forma de CreateProyectoDTO
  ...
  // Este useEffect ya está correcto
  useEffect(() => {
        ...
        <DialogContent dividers>
          <Stack spacing={3} sx={{ mt: 1 }}>
            ...
            <Box>
              ...
              <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, gap: 2 }}>
                <TextField
                  ...
                  // ❗ CORRECCIÓN 5: Usar 'text' para el input, ya que el DTO es string
                  // (type="number" funciona, pero 'text' es más consistente con el DTO/Validación)
                  type="text" 
            ...
            <Box>
              ...
              {/* Este componente ahora está importado */}
              <SingleImageUpload
                image={image}
                onChange={setImage}
                maxSizeMB={15}
