// src/pages/Proyectos/ProyectoDetail.tsx (Corregido)
// ═══════════════════════════════════════════════════════════
...
// --- 1. Servicios y Tipos ---
// ❗ CORRECCIÓN 1: Importar proyectoService con llaves {}
...
import type { ProyectoDTO, ProyectoEstado } from '../../../types/dto/proyecto.dto';
...
// ❗ CORRECCIÓN 2: Usar el tipo importado 'ProyectoEstado'
const statusDisplayMap: Record<ProyectoEstado, { label: string; color: "info" | "success" | "default" }> = {
...
const ProyectoDetail: React.FC = () => {
  ...
  const { data: project, isLoading, error } = useQuery<ProyectoDTO | null, Error>({
    ...
    // ❗ CORRECCIÓN 3: El método se llama 'getProyectoActivoById'
    queryFn: () => proyectoService.getProyectoActivoById(Number(id)),
    enabled: !!id,
    retry: false,
  });

  // ❗ CORRECCIÓN 4: Llamar al servicio real de cuotas
  const { data: cuotasArray, isLoading: isLoadingCuota } = useQuery<CuotaMensualDTO[], Error>({
    queryKey: ['cuotasMensuales', id],
    // (Asumiendo que el método para obtener cuotas es 'getByProyectoId')
    queryFn: () => cuotaMensualService.getCuotasByProyecto(Number(id)), 
        ...
        {project && (
          <>
            ...
            {(() => {
              ...
              // ❗ CORRECCIÓN 5: Convertir string (DECIMAL) a número antes de formatear
              const montoDisplay = isInvestor
                ? Number(project.monto_inversion).toLocaleString()
                : cuotaMensual ? Number(cuotaMensual.valor_mensual_final).toLocaleString() : 'N/A';
...
// ❗ CORRECCIÓN 6: Eliminar la función stub
// (La función 'getCuotasByProyectoId' de abajo fue eliminada)