...
// ❗ CORRECCIÓN 1: Importar el servicio (asumiendo que es nombrado)
import { usuarioService } from '../../../Services/usuario.service';
import type { UpdateProfileDTO, UsuarioDTO } from '../../../types/dto/usuario.dto';
import { PageContainer } from '../../../components/common/PageContainer/PageContainer';
// ❗ Asumo que tu AuthContext exporta 'AuthenticatedUser'
...
const Perfil: React.FC = () => {
  ...
  const mutation = useMutation({
    // ❗ CORRECCIÓN 2: Llamar a 'updateMyProfile'
    mutationFn: (data: UpdateProfileDTO) =>
      usuarioService.updateMyProfile(data),
    
    onSuccess: (updatedUser: UsuarioDTO) => {
      if (user) { 
        const userCompleto: AuthenticatedUser = {
          ...user, 
          ...updatedUser,
        };
        updateUserContext(userCompleto); 
      }
      setIsEditing(false);
      setSuccessOpen(true);
    },
        ...
        >
          {!isEditing ? (
            ...
            <form onSubmit={formik.handleSubmit}>
              <Stack spacing={2}>
                {(mutation.isError || formik.status) && (
